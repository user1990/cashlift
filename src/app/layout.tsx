import type { Metadata, Viewport } from "next";
import { connection } from "next/server";
import { SITE_META } from "@/modules/marketing/site";
import { SITE_URL } from "@/services/site";
import { MAIN_CONTENT_ID } from "@/ui/components/layout/MainContent";
import { ClientTelemetry } from "./ClientTelemetry";
import "./globals.css";

type RootLayoutProps = {
	children: React.ReactNode;
};

export const metadata: Metadata = {
	metadataBase: new URL(SITE_URL),
	title: "CashLift",
	description: SITE_META.defaultDescription,
	icons: {
		apple: "/brand/cashlift-icon-180.png",
		icon: "/brand/cashlift-icon.svg",
	},
	openGraph: {
		description: SITE_META.defaultDescription,
		images: [
			{ alt: "CashLift cash decision workspace", height: 1_500, url: "/brand/hero-finance-command.png", width: 2_400 },
		],
		type: "website",
	},
};

export const viewport: Viewport = {
	themeColor: "#040a12",
};

export const instant = false;

export default async function RootLayout({ children }: RootLayoutProps) {
	await connection();

	return (
		<html lang="en" className="h-full" data-scroll-behavior="smooth">
			<body className="grid min-h-full grid-cols-[1fr_0px] antialiased" id="top">
				<a
					href={`#${MAIN_CONTENT_ID}`}
					className="focus-ring fixed top-4 left-4 z-50 -translate-y-24 rounded-md border border-primary bg-shell-elevated px-3 py-2 font-medium text-m text-shell-foreground shadow-shell outline-none transition-transform duration-150 focus-visible:translate-y-0"
				>
					Skip to content
				</a>

				{children}

				{!!process.env.VERCEL_ENV && <ClientTelemetry />}
			</body>
		</html>
	);
}
