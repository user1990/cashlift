...
type MarketingLeadCaptureAction = {
	href: string;
	label: string;
	variant?: "primary" | "secondary";
};

type MarketingLeadCaptureSectionProps = {
	description: ReactNode;
	headline: string;
	primaryAction: MarketingLeadCaptureAction;
	secondaryAction: MarketingLeadCaptureAction;
};

export const MarketingLeadCaptureSection = ({
	description,
	headline,
	primaryAction,
	secondaryAction,
}: MarketingLeadCaptureSectionProps) => (
	<Panel
		as="section"
		variant="glass"
		className={cn(
	...
	>
		...
		<div className="relative flex flex-col">
			<header className="mb-3">
				...
				<h2 className="mt-2 text-l+ text-shell-foreground">{headline}</h2>
			</header>

			<p className="text-m text-shell-muted leading-6">{description}</p>

			<div className="mt-5 grid gap-2 sm:grid-cols-2">
				<ActionLink href={primaryAction.href} className="w-full">
					{primaryAction.label}
				</ActionLink>

				<ActionLink href={secondaryAction.href} variant={secondaryAction.variant ?? "secondary"} className="w-full">
