...
export const PricingPage = () => (
	<main className="mx-auto max-w-[1180px] px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
		...
		<ul className="mt-10 grid auto-rows-fr items-stretch gap-4 lg:grid-cols-3">
			{PRICING_PLANS.map(({ description, features, highlighted, name, price }) => (
				<li key={name} className="flex">
					...
					>
						<div className="flex items-start justify-between gap-4">
							<div>
								<h2 className="text-4xl+ tracking-normal text-shell-foreground">{name}</h2>

								<p className="mt-3 text-m font-medium leading-6 text-shell-muted">{description}</p>
							</div>
