...
function getApprovalsHeadline(pendingCount: number, pendingAmountCents: MoneyCents) {
	...
	if (pendingCount === 1) {
		return `1 spend request totaling ${formatPreciseCompactCurrency(pendingAmountCents)} needs a decision`;
	}

	return `${pendingCount} spend requests totaling ${formatPreciseCompactCurrency(pendingAmountCents)} need a decision`;
}
